...
def main(args: Optional[argparse.Namespace] = None) -> None:
    ...
    # Remove junk files
    removed_count = remove_junk(junk_items)
        ...
        for root, dirs, files in os.walk(directory_path):
            ...
            for dir_name in dirs[:]:  # Copy to allow modification
                if dir_name in junk_dirs:
                    junk_items.append(root_path / dir_name)
                    dirs.remove(dir_name)  # Don't descend into junk dirs

            # Check for junk files
            for file_name in files:
                if is_junk_file(file_name, junk_files):
                    junk_items.append(root_path / file_name)
    else:
        for entry in os.scandir(directory_path):
            if entry.is_dir() and entry.name in junk_dirs:
                junk_items.append(Path(entry.path))
            elif entry.is_file() and is_junk_file(entry.name, junk_files):
                junk_items.append(Path(entry.path))
...
def remove_junk(junk_items: List[Path]) -> int:
